...
const VISIBLE = 7;
// En dash, no surrounding spaces: the spaces are what let a range wrap mid-value.
const spread = (p: HistoryPoint) => p.min == null && p.max == null ? '—' : `${money(p.min)}–${money(p.max)}`;
export default function GMPChart({ history = [] }: { history?: HistoryPoint[] }) {
  ...
  {/* Grid rows, not a table: three fixed tracks keep date, median and range on one line down to 320px. */}
  <ul className="daily-gmp" aria-label="Daily GMP by date">{(showAll ? days : days.slice(0, VISIBLE)).map(p => <li key={p.date}>
    <span className="daily-date">{formatDate(p.date)}</span><strong className="daily-median">{money(p.median)}</strong><span className="daily-range">{spread(p)}</span>
  </li>)}</ul>
  {days.length > VISIBLE && <button className="daily-toggle" aria-expanded={showAll} onClick={() => setShowAll(!showAll)}>{showAll ? 'Show fewer days' : `Show all ${days.length} days`}</button>}</div>;
}
